"use client"
import { AlertTriangle, ChevronLeft, ChevronRight, Clock, Gauge, ThermometerSnowflake } from "lucide-react"
              ...
              {alerts.length === 0 ? (
                ...
                <div className="space-y-4">
                  {alerts.map((alert) => (
                    <div
                      key={alert.id}
                      className={`rounded-lg border p-3 transition-colors cursor-pointer ${!alert.read ? "border-yellow-200 bg-yellow-50 hover:bg-yellow-100" : "hover:bg-gray-50"
                        }`}
